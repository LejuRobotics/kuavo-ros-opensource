...
- [接口使用文档](#接口使用文档)
    ...
    - [使用指南](#使用指南)
    - [`/get_used_audio_buffer_size` 获取音频播放缓冲区大小](#get_used_audio_buffer_size-获取音频播放缓冲区大小)
    - [`/get_used_audio_buffer_size` 获取音频播放缓冲区大小](#get_used_audio_buffer_size-获取音频播放缓冲区大小-1)
...
### `/get_used_audio_buffer_size` 获取音频播放缓冲区大小
...
| message | string | 返回音频播放缓冲区使用情况（当前队列中音频块的数量） |
...
- **类型**: `kuavo_msgs/playmusic`

**请求字段:**

| 字段 | 类型 | 描述 |
| --- | --- | --- |
| music_number | string | 音乐文件的名称或编号。可以是文件路径或预定义的音乐编号 |
| volume | int64 | 音乐的音量，范围通常为 0 到 100 |
...
- **类型**: `kuavo_msgs/recordmusic`

**请求字段:**

| 字段 | 类型 | 描述 |
| --- | --- | --- |
| music_number | string | 音乐文件的编号。用于标识要录制的音乐文件 |
| time_out | int64 | 超时时间，以秒为单位。指定录制操作的最大持续时间 |
...
| header.seq | uint32 | 序列号 |
| header.stamp | time | 时间戳 |
| header.frame_id | string | 坐标系 ID |
| orientation | geometry_msgs/Quaternion | 方向四元数 |
| orientation.x | float64 | 四元数 x 分量 |
| orientation.y | float64 | 四元数 y 分量 |
| orientation.z | float64 | 四元数 z 分量 |
| orientation.w | float64 | 四元数 w 分量 |
| orientation_covariance | float64[9] | 方向协方差矩阵 |
| angular_velocity | geometry_msgs/Vector3 | 角速度 |
| angular_velocity.x | float64 | 角速度 x 分量 |
| angular_velocity.y | float64 | 角速度 y 分量 |
| angular_velocity.z | float64 | 角速度 z 分量 |
| angular_velocity_covariance | float64[9] | 角速度协方差矩阵 |
| linear_acceleration | geometry_msgs/Vector3 | 线性加速度 |
| linear_acceleration.x | float64 | 加速度 x 分量 |
| linear_acceleration.y | float64 | 加速度 y 分量 |
| linear_acceleration.z | float64 | 加速度 z 分量 |
| linear_acceleration_covariance | float64[9] | 加速度协方差矩阵 |
...
| fields | sensor_msgs/PointField[] | 点字段信息 |
| fields[].name | string | 字段名称 |
| fields[].offset | uint32 | 字段偏移 |
| fields[].datatype | uint8 | 数据类型 |
| fields[].count | uint32 | 字段计数 |
...
| 字段 | 类型 | 描述 |
| --- | --- | --- |
| control_mode | int32 | 要设置的控制模式编号。有效值为 0、1 或 2 |
...
| mode | int32 | 返回实际设置的控制模式编号 |
| message | string | 包含关于操作结果的详细信息的消息 |
...
| 字段 | 类型 | 描述 |
| --- | --- | --- |
| times | float64[] | 时间列表，表示每个姿态的目标时间点 |
| values | float64[] | 关节角度列表，表示手臂在每个时间点的目标关节角度 |
| frame | int32 | 参考坐标系。0=保持当前坐标系, 1=世界坐标系(基于odom), 2=局部坐标系, 3=VR坐标系, 4=操作世界坐标系 |
...
| name | string[] | 关节名称列表，假设有 14 个关节，名称为 `"arm_joint_1"` 到 `"arm_joint_14"` |
| position | float64[] | 当前关节位置列表 |
...
| 字段 | 类型 | 描述 |
| --- | --- | --- |
| joint_data | float64[] | 包含头部偏航角和俯仰角的列表。偏航角范围为 [-30, 30] 度，俯仰角范围为 [-25, 25] 度 |
...
| --- | --- | --- |
| header | std_msgs/Header | 消息头，包含时间戳等信息 |
| left_hand_position | uint8[] | 左手位置，包含6个元素，每个元素的取值范围为[0, 100], 0 为张开，100 为闭合 |
| right_hand_position | uint8[] | 右手位置，包含6个元素，每个元素的取值范围为[0, 100], 0 为张开，100 为闭合 |
...
| 字段 | 类型 | 描述 |
| --- | --- | --- |
| linear.x | float64 | 基于当前位置的 x 方向值，单位为米 (m) |
| linear.y | float64 | 基于当前位置的 y 方向值，单位为米 (m) |
| linear.z | float64 | 增量高度 (m) |
| angular.x | float64 | 未使用，设置为 0 |
| angular.y | float64 | 未使用，设置为 0 |
| angular.z | float64 | 基于当前位置旋转（偏航）的角度，单位为弧度 (radian) |
...
| 字段 | 类型 | 描述 |
| --- | --- | --- |
| linear.x | float64 | x 方向线速度 (m/s) |
| linear.y | float64 | y 方向线速度 (m/s) |
| linear.z | float64 | 增量高度 (m) |
| angular.x | float64 | 未使用，设置为 0 |
| angular.y | float64 | 未使用，设置为 0 |
| angular.z | float64 | yaw 方向角速度 (radian/s) |
...
| FTsensor_data     | kuavo_msgs/FTsensorData    | 足底六维力/力矩传感器数据 (Fx, Fy, Fz, Mx, My, Mz) |
...
| frame | int32 | 参考坐标系。0=保持当前坐标系, 1=世界坐标系(基于odom), 2=局部坐标系, 3=VR坐标系, 4=操作世界坐标系, 5=关节空间 |
...
| elbow_cost_scale | float64 | 手肘代价缩放系数（相对于 pos_cost_weight）。==0 使用默认值(0.1), <0 禁用手肘约束, >0 直接使用该值 |
...
| 字段 | 类型 | 描述 |
| --- | --- | --- |
| timestamp_ms | int64 | 时间戳 |
| is_high_confidence | bool | 数据置信度标志 |
| is_hand_tracking | bool | 手部追踪状态标志 |
| poses | PoseInfo[] | 包含所有骨骼的位姿数据列表，每个骨骼包含： |
| poses[].position | geometry_msgs/Point | 骨骼位置 (x, y, z) |
| poses[].orientation | geometry_msgs/Quaternion | 骨骼方向四元数 (x, y, z, w) |
...
def dexhand_touch_state_callback(msg):
    left_hand_normal_force1 = msg.left_hand[0].normal_force1
    # 打印当前触觉传感器1的法向力状态
