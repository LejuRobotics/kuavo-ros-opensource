...
- [接口使用文档](#接口使用文档)
    ...
    - [`/stop_music` 停止播放](#stop_music-停止播放)
...
- **类型**: `std_srvs/TriggerResponse`
- **字段**:
  - `success` (bool): 表示请求是否成功。
  - `message` (str): 当前缓冲区剩余字节数。
...
播放指定音频文件，支持追加排队或原子化打断当前播放立即播新。

2. 请求格式

- **类型**: `kuavo_msgs/playmusic`
- **字段**:
  - `music_number` (str): 音频文件名，需位于 `music_path` 指定的目录下。
  - `volume` (int): 音量 0–100。100 为原声，0 近静音，中间按 dB 对数缩放。
  - `immediate` (bool): `True` 先同步停止当前播放再加载新文件（打断+播新一次完成，无竞态）；`False`（默认）追加到缓冲按队列顺序播放，不打断当前。

3. 响应格式

- **类型**: `playmusicResponse`
- **字段**:
  - `success_flag` (bool): 请求是否成功。
...
req.music_number = "1_挥手.mp3"
req.volume = 80
req.immediate = False  # 追加播放；需立即切换语音时设 True
client(req)
```
...
> 兼容入口：发布同名话题 `/stop_music`（`std_msgs/Bool`，`True` 触发停止）也可停止播放，但不返回结果，建议用服务。
...
- **joint_data** (list of float): 包含头部偏航角和俯仰角的列表。偏航角范围为 [-88, 88] 度，俯仰角范围为 [-25, 25] 度。

4. **注意**

此话题发送的是**绝对目标角度**。遥控器（H12/G12）已改用速度接口 `/cmd_head_vel`，不再通过此话题控制头部；此话题保留给 SDK/VR 等直接角度控制场景。
...
### `/cmd_head_vel` 头部关节角速度控制

1. 功能描述

`/cmd_head_vel` 话题用于通过发布**瞬时角速度**（deg/s）控制机器人头部运动。接收侧（CDM/WBC）做 sticky 积分：持续收到非零速度时累积角度，收到零速度或 0.3s 超时后停止。
...
- **joint_data** (list of float): 长度为 2。
...
### `/cmd_head_delta` 头部关节增量控制

1. 功能描述

`/cmd_head_delta` 话题用于发送**一次性相对位移**（deg）。oneshot 语义：每条消息的增量被消耗一次后清零。
