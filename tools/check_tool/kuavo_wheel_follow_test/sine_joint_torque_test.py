...
"""
正弦波关节力矩控制测试脚本
基于 S61JointController 对选择关节进行正弦力矩控制测试
"""
...
class SineJointTorqueTester:
    ...
    def set_joint_sine_params(self, joint_id, amplitude, frequency, phase=0.0):
        ...
        rospy.loginfo(f"设置关节[{joint_id}]: 力矩振幅={amplitude:.3f}Nm, 频率={frequency:.2f}Hz, 相位={phase:.3f}rad")
        return True
    ...
    def run_sine_test(self):
        ...
        # 显示关节参数
        rospy.loginfo(f"  关节[{self.test_joint_id}]: 力矩振幅={self.sine_amplitude:.3f}Nm, 频率={self.sine_frequency:.2f}Hz, 相位={self.sine_phase:.3f}rad")
    ...
    def print_joint_status(self, t, target_tau):
        ...
        if self.test_joint_index is not None:
            ...
            status_str = f"[{t:.1f}s] 关节[{self.test_joint_id}]: 目标力矩={target:.3f}Nm, 实测力矩={measured:.3f}Nm, 误差={error:.3f}"
