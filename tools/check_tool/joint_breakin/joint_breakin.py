...
class KuavoUnifiedBreakin:
    def __init__(self):
        ...
        self.leg_breakin_script_kuavo5 = self.current_dir / "ec_master_tools" / "kuavo5_leg_breakin.py"
        self.leg_breakin_script_roban2 = self.current_dir / "ec_master_tools" / "roban2_leg_breakin.py"
    ...
    def detect_leg_script_version(self):
        ...
        if self.robot_version is None:
            ...
            # 根据版本号自动选择
            version_num = int(self.robot_version) if self.robot_version.isdigit() else 0
    ...
    def run(self):
        ...
        if self.leg_breakin_script:
            script_name = self.leg_breakin_script.name
            if "roban2" in script_name:
                self.print_colored("使用 Roban2 腿部磨线脚本（适用于版本13-14）", Colors.BLUE)
            elif "kuavo4" in script_name:
                self.print_colored("使用 Kuavo4 腿部磨线脚本（适用于版本40-49）", Colors.BLUE)
            elif "kuavo5" in script_name:
                self.print_colored("使用 Kuavo5 腿部磨线脚本（适用于版本50-51）", Colors.BLUE)
        print()
