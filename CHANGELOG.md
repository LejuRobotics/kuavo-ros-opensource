...
## 文档相关
- 补充手臂/头部限位标定堵转保护说明与 cali_arm 机械零位要求，[文档链接](./readme.md)
...
- 新增 G12 航空箱坐/站操作说明及姿态参考图，[文档链接](./docs/2快速开始/G12手柄控制文档/Kuavo%205-W%20G12操作.md)
- 补充轮臂 v62/v63 下肢限位标零说明，完善 readme 通用标定章节与限位标零入口，[文档链接](./docs/3调试教程/Kuavo%205-W%20全身零点标定.md)

## 新增功能
- 硬件限位标定新增手臂堵转检测与失能保护：标定模式下全程监测瑞沃手臂关节，(跟随误差>2° 且 速度<1°/s 且 力矩>阈值) 持续 0.2s 即判定堵转并失能手臂，堵转/超时/回程失败等异常统一回滚本次零点修改、禁止进入保存确认避免写入错误零点
- kuavo_v53/v54/v55 新增 amp_controller_2（与 vmp_controller 共用 enabled 开关，防止从 VMP 切 MPC 时卡死）
- 新增上下楼梯启动 launch，[文档链接](./src/humanoid-control/humanoid_controllers/launch/load_kuavo_real.launch)
- 新增 5W 机器人 5G/网口 DHCP 一键配置脚本
- 延迟诊断脚本新增 PICO+绝对式+人形链路，优化互相关计算异常处理
- Quest3 绝对式上臂快速模式：左前扳机 + X 上升沿在普通快速模式（quickMode=0）与上臂快速模式（quickMode=2）间切换，仅 Python 绝对式 IK 启动
- Quest3 遥操新增控制延迟诊断能力，划分 joystick/UDP 延迟分段统计
- 人形绝对式 VR IK 改为共享内存传递，新增 VR 处理层、IK 解算层及各段链路延迟诊断脚本与 bag 曲线提取工具
- 硬件工具 Hardware_tool.py 新增一键安装/卸载 WiFi 热点功能，卸载前先停止热点服务
- 腕部关节到达限位时速度处理由硬清零改为阻尼归零
- lejuclaw 新增显式高低频模式、断流看门狗与 /leju_claw_target 观测话题
- 头部控制默认跟踪 VR 设备运动
- CAN 电机控制帧新增 RT 直发路径（绕过 ring 队列），并新增发送耗时（CAN-LAT enq_to_send）分位统计
...
- WS SDK 新增速度斜坡控制（ramp_duration），消除搬箱倒退摔跤
- WS SDK launch 新增 real 参数支持自动切换仿真/实机配置
- 5W 增量单手遥操新增坐姿单手控制躯干能力，单手即可控制折叠臂升降与前后倾、腰部左右旋转，另一只手可自由控制底盘或手臂，[使用文档](./docs/轮臂V1.4开发文档/轮臂单手增量摇操躯干说明.md)
- 鲁班2.2 新增 AMP 行走动手步态，支持行走中下蹲弯腰
...
- 重构 WiFi 上报脚本，通过监听 IP 变化自动上报 WiFi 信息
- 添加 56 版本灵心巧手肩部偏移配置
- 轮臂 WBC 新增 /enable_control 服务与状态门控冻结逻辑，支持躯干/左右臂独立使能控制，[文档链接](./docs/轮臂相关接口文档说明.md)
- 新增人形与 5 代轮臂搬箱子案例源码及测试脚本，[使用教程](./src/demo/pickbox/搬箱子案例使用教程.md)
- 新增轮臂视觉标定零点方式，适配 62/63 版本标定 URDF 与单部位标定
- Roban2.2 AMP 行走新增速度档位调节（十字键上/下键升降档），摇杆停止后持续发零速指令提升平滑性
- Roban2.2 AMP 模型优化并补充 craic 比赛动作
- VR 节点启动新增是否开启腰部控制的参数开关
- v63 轮臂适配 VR 增量遥操作配置与 Drake 模型
- 新增 53/54 AMP 跑步与崎岖路面步态模型及配置（默认不开启）
- KUAVO5 新增座椅起身功能，包含完整起身流程、收脚动作、堵转保护与起身刚度调整
- 倒地起身新增后倒（仰卧）姿态支持，前倒/后倒由控制器自动识别
...
- 修复轮臂多场景腰部异常升高：未解锁位置跟随转头、未激活、增量模式切换回零升高，并锁定切换关闭瞬间 chest pitch、修复站立状态位置跟随细分关闭失效
- 轮臂增量式发布下肢关节命令平滑系数 alpha 改回 0.01
- 新增上肢小臂碰撞对约束；优化肩部收紧成本条件与滤波系数、实时更新收紧锚定参考；修复 play_back_wheel 无法回放新 bag
- dance 切换 amp 新增播后静置逻辑与双插值
- 撤回基类新增的失能虚函数，修复手臂磨线节点启动后立即退出
- 修复 pdo kp/kd 参数，行走最大速度设为 0.4，并新增深度流异常时的速度指令清零保护
- 微调 amp_wild 默认站立姿态，解决轻推容易后仰
- 修复人形绝对式电机跟踪层延迟
- 移除 hardware_nodelet 重复初始化，避免硬件二次加载失败
- 修复 AMP 长时站立后切回 MPC 时腰部异常转动
- 修复 RL 切回 MPC 偶尔出现腰部异常转动
- 增大手臂移动速度
- 修复 AMP 踏步时 LinkerHand 拇指内扣（#4033）
- 修复 humanoid 与 mobile_manipulator 控制器 TopicLogger 发布队列 map 并发访问竞态
- 修复座椅起身准备状态上报时机：在就绪站姿才上报 seat prep readiness
- 修复 5W 转腰无法移动：新增 waist yaw 障碍函数，抑制手臂摆动时腰部随动
- 修复整机关机稳定性：对齐硬件生命周期状态转换，限制改动到硬件停止路径
- 修复 53/55 版本磨线异常问题
- 修复执行旧手臂二进制文件的问题
- 更新 52 机型 amp 配置，清理 amp 步态多余模型文件
- 暂缓 AMP-AMPwild 切换后的指令执行，平滑切换效果
- 修复 kuavo5 AMP 与 MPC 模式下手平举时按 X+A 解锁手臂、手臂到人手位置时概率性快速抽动的问题；AMP 将模式切换请求从 ROS 回调线程转交控制线程统一处理，MPC 重新整理模式切换插值逻辑
- 恢复 5W 单手遥操 single X+A 激活 #4019
- 修复可视化发布频率：统一节流降低 /tf 等话题频率，减轻跨机链路负载
- 修复 MoRE RT 直发交接评审问题：回滚不完整、魔法数 sleep、无 quiesce 握手，并延迟到 init 完成后接管，修复启动手臂跳变到零点
- 修复 Quest3 joystick 与 UDP 生命周期、仅 joystick 接收周期、指令快照保活与 UDP 位姿积压等稳定性问题，修正控制延迟诊断测量
- 修复 pico 遥操头部跟随问题（head_control_mode）
- 修复 pico_gmr 在 AMP 模式下 8/11 与 8/16 Joy 的支持
- 修复校准模式下 hardware_status_ 延迟到腿绷直后才设为 calibrate
...
- 修复 M1+M2 动作执行期间误触发步态切换（踏步/站立/控制器切换）
- 修复 WS SDK 多项问题：手臂动作卡顿/振荡、walk→stance 切换卡顿、轮臂 is_gait() 判定异常、腰关节反馈数据始终为 0
- 修复自研CAN版本夹爪无法通过配置文件动态调整参数，优化退刀槽后夹爪的开爪限位与 KD 参数
- 修复手臂磨线选择"仅启动手臂磨线"模式时只执行 1 轮的问题
- 修复软暂停/恢复未同步调用底盘 /move_base/set_dispatch_mode 的问题
- 修复 Drake 解算脚掌位置未修正为脚掌中心的问题
- 修复 s62 模型底座高度实测比仿真高 3cm 的问题
- 修复手臂零点话题发布值与配置文件不一致的问题
- 修复 kuavo5 v52 无法切换步态的问题
- 修复 rviz 中机器人模型闪烁的问题
- 修复轮臂部分姿态下下肢抖动的问题
- 修复 Roban2.2 倒地起身复位后腿部不使能的问题
- 修复 MPC 启动时速度限制未重置导致 Roban 行走异常
- 修复 kuavo_v56 头部 Ruiwo kpkd 参数与 v53/v54 不一致
- 恢复 kuavo5 手臂正逆解默认容差（6 代轮臂保持紧容差）
- 修复 s56 MPC 发散问题，增大膝盖力矩限幅
- 修复 Roban2.2 跳舞后按 X 错误切换到 MPC 步态，修正为 AMP 步态
- 修复 amp_hand_controller 弯腰下蹲模式导致行走控制器切换阻塞
- 关闭 v54 上楼梯控制器避免无深度数据时切入异常（master 热修复）
- 修复 H12 启动监控误判导致的重启循环及 start_way 残留误杀问题
- 修复手动启动 H12 时 joy_node 与服务侧争抢同名节点的问题
- 修复 Roban2.2 执行 tact 动作后手臂模式未恢复的问题
- 修复跳舞无背景音乐的问题（共用 dance_trajectory_state publisher）
- 修复手臂控制折叠臂 x 方向运动卡顿的问题
- 修复轮臂控制积分累积与限幅不匹配的问题
- 修复 rosbag 节点 nodelet 销毁时循环引用导致的崩溃
- 修复跨秒生成多个 stdout.log 目录的问题
- 修复夹爪轮臂 Robot Module 版本归属错误
- 对齐 s17 头部与脚踝参数，回退末端坐标参数
- 修复 Quest VR 转腰回中 1s 内误切 AMP 步态的问题
- 修复轮臂 VR 后退速度与前进不一致的问题
- 拆分 G12 手柄与 VR 底盘速度限制，统一 v61-63 轮臂 VR 横向速度限制
- 修复 Roban（major 版本号为 1）的控制判断及 tact 动作的机器人版本兼容判断
...
- 修复 AMP 自动切换逻辑异常问题：避免非 amp/amp_wild 模型被强制切至 amp，及五代 AMP 自动模型切换影响其余机型上肢动作
- 修复人形 VR 增量遥操手臂模式从 2 切换到 0/1 时未退出增量主线程的问题
- 修复站立后 gait_fre 置零逻辑导致模型站立歪斜的问题
- 修复座椅起身速度过快、MPC 初始化过快导致的稳定性问题（回退调试参数）
- 修复搬运退出后 yaw 甩回旧值及倒地起身后未正确切回 AMP 的问题（issue #3614）
- 修复倒地开机功能失效的问题，相关 param 清理改由 START+BACK 触发（issue #3616）
...
- 优化 OCS2 进程管理，避免残留进程影响下次启动
- 部署脚本支持 GitCode 仓库源，统一版本注册表
- rosbag 日志新增乐聚夹爪相关调试话题
...
- 禁用编译时自动更新 URDF/XML 质量，避免构建时意外修改模型文件
- 优化鲁班 amp_hand_controller 直行急停参数
