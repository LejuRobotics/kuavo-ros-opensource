...
## 新增功能
- 新增 service 控制强脑灵巧手进入 turbo 模式（支持左手、右手、双手三个独立接口），[文档链接](./docs/运动控制API.md)
- 新增轮臂初始化服务，启动时固定手臂模式为 1，防止 X+A 误切换状态
- Kuavo 五代 v53 / v55 机型支持舞蹈功能
- 增加配网工具，支持机器人热点网页配置与部署，安装脚本运行时自检 kuavo-tools 版本，[文档链接](./tools/linux_wifi_hotspot/readme.md)
- 5W 轮臂机型适配 G12 遥控器播放动作与语音功能
- Roban2.2 舞蹈控制器支持舞蹈执行时同步播放背景音乐，并发布舞蹈状态消息（名称/进度/状态）
- 轮臂安全模块拆分为静态安全（关节限位）和动态安全（速度/加速度限制），支持预设限制和扭矩约束自计算
- 轮臂新增进入 MPC 前手臂自动收回预动作（肘关节旋转 30 度后复位），重构关节指令限位器
- 关节限位保护模式改为超出限位 + 扭矩/速度检查后断电，替代直接掉使能
- 新增多舞蹈切换服务接口，支持按名称或下标切换舞蹈控制器和查询已加载舞蹈列表，[文档链接](./docs/RL控制框架ROS接口文档.md)
- Roban2.2 新增5个舞蹈遥控器按键触发绑定（LT+A 芭蕾/LT+B 新疆舞/LT+X 爱情鸟/RB+A 芭啦芭啦樱之舞/RB+Y lonelydance），MPC 或 AMP 下均可直接触发
- VR 启动根据机器版本自动判断是否启动 wheel_ik，轮臂实机默认启用
- Roban 版本 RT+X 保留为动作组合键，不再触发控制器切换
- Kuavo5 depth_loco_controller 切换前新增深度话题可用性后台校验（发布状态/数据时效/频率下限），不满足时拒绝切换
- 新增 5 代机器人夹爪 URDF（biped_s200053/s300053/s200062/s300062），支持完整 TF 链
- 新增灵心巧手 L6/O6 在 Mujoco 中的控制和 ROS 接口适配，实现 RVIZ 可视化及 TF 显示
- 新增 Roban2.2(s17) 俯身版倒地起身功能
- 新增相机标定和动捕标定功能，[相机标定文档](./src/Camera_Calibration/README.md)，[动捕标定文档](./tools/arm_accuracy_calibration/README.md)
- 轮臂新增 IK 可达性检查服务和末端执行后静态误差计算服务
- 新增灵心巧手 L6/O6 版本 URDF 模型适配及 Mujoco 仿真支持，基于49版本构建搭建框架并组装模型到末端关节
- 轮臂 MPC 优化：改进笛卡尔约束与屏障函数策略，优化 focusTorso/focusEe 模式自动切换及切换时躯干平滑过渡
- 新增轮臂负载快速搬箱案例和手臂期望力控制功能
- 新增 Roban2.2 AMP 行走功能
- 新增 55 版本机器人型号，修复五代版本腰部和头部摄像头坐标
...
## 修复问题
- 修复手臂关节零点异常可能导致电机堵转/烧毁的风险
- 修复 Roban2.2 倒地起身掉使能状态下按 RB+X 时只有手臂复位、腿部不复位的问题
- 修复 kuavo5 VR 的 AMP 步态下按 X+A 无法复位手臂的问题
- 修复 5W 机器人在运行状态下按 X+A 无法激活增量遥操作
- 修复 5W H12/北通躯干控制模式下，单独控制下肢关节抬起到一定角度时机器剧烈抖动
- 修复 rosbag 分包后回放时 RViz 中手指/手臂模型位置异常的问题
- 修复 H12 控制服务自动部署后启动早期偶发的报错，并优化关键节点健康检查与异常自动恢复机制
- 修复 RL/AMP 步态下播放手臂动作时开头出现异常抽动/下放的问题
- 修复 kuavo5 双足机器人长按 A 进入 VR 模式时被 H12 通道透传误触发踏步（walk）的问题
- 修复 MuJoCo 仿真强脑手大拇指 CMC/MCP 关节读写映射范围不一致的问题
- 修复轮臂 VR 模式归 1 时反复触发快速模式服务导致卡顿
- 修复 kuavo5 VR 手臂模式切换过程中机器手臂抖动（模式切换期间 IK 初始猜测被反复重置，导致与 MPC 预测不一致）
- 修复 kuavo5 增量遥操 X+A 复位手臂期间再次按下 X+A 导致手臂快速抽搐复位、并直接切回增量模式的问题
- 修复 Pico 手臂复位锁定期间设置头部为 fixed 时，解锁后头部错误恢复主手跟踪而不是保持正前方的问题；Quest3 路径同步处理
- 修复五代 v53/v55 灵心巧手摆臂时与大腿碰撞的问题
- 修复 V55 机器人通过桌面软件执行动作失败
- 修复 RVIZ 中 rosbag 回放时手指脱离手掌的问题，以及 O6 手话题仿真不显示
- 修复 kuavo_sdk 键盘控制脚本缺少相关服务时卡死
- 修复轮臂底盘速度控制时指令为 0 会强制切出到位置控制
- 修复 VR 实物启动时手臂模式未与硬件同步导致增量激活异常，启动站立改为长按右手柄 A 约 0.35s
- 修复 PICO 手臂复位/解锁后头控与主手模式恢复错误
- 修复 V56 从 AMP 步态切换时卡在不可用控制器导致无法切回 MPC
- 修复 5W-H12 底盘移动时头部抖动问题
- 修复键盘控制行走时启动阶段步态快速振荡及停步后误重启
- 旧版轮臂 VR 移除低延迟模式快捷键和左手扳机切控制器，避免误触发不支持的功能
- Kuavo Humanoid SDK 修复 control_arm_joint_positions 接口调用耗时过高无法达到 100Hz 的问题
- 修复增量遥操松开侧扳机后腰部 yaw 不跟随
- 修复 VR 控制切换手臂模式时手臂到达速度过快（跳过的轨迹被错误存储导致缺少过渡）
- 修复键盘控制 IK 鲁班机型肘部姿态不合理，自动计算合适的肘部位置并调小软约束权重
- 修复 5W 轮臂 VR 开启/退出低延迟模式时必定报错（调用了轮臂控制器中未注册的 wbc 服务），改为调用 enable_lb_arm_quick_mode
- 修复轮臂 odom→base_link tf 双源冲突导致模型跳动（实物和回放模式）
- 修复轮臂执行时间慢慢变长的问题
- 修复 5-W 重启 VR 后需按两次 X+A 才能解锁手臂
- 修复绝对式 VR 无法移动的问题，以及 V55 版本控制器切换时抖动
- 修复 5-W G12 躯干控制模式下肢 1/3 号电机运动速度过快（G12 高频输入导致积分增益累加过快）
- 修复轮臂多项问题：躯干初始位姿赋值、底盘速度指令增量式、MPC 循环频率对齐
- 修复轮臂人型节点检测误报、VR 速度限制与手柄不一致
- 修复 VR 头部控制模式切换异常（退出手臂控制后模式被错误恢复）
- 修复 4pro 启动半身模式时手臂抽搐后程序崩溃问题
- 修复轮臂多个状态切换问题：G12 遥控器 EF 拨杆屏蔽动作播放、VR 按 A 无法切到运行状态、H12 控制中误进入 walk/trot 导致无法进入 VR
- 修复缩腿未完成时提前按站立导致机器状态与遥控器状态不一致，增加 ready_stance 前置检查
- 修复 5W 轮臂北通遥控器缺少 Start 键映射导致无法从准备状态切换到运行状态
- 修复手臂插值运动抖动问题，插值过程中差值异常增大时不更新
- 修复轮臂 pytree 案例下发指令前未设置 focus_ee 导致下肢不运动
- 修复轮臂躯干笛卡尔位置指令按 focus 手臂末端执行时上肢震动问题
- 修复轮臂 VR 模式下无法通过按键组合进入躯干控制模式
- 修复 4pro 机器人抬手遥操时脚踝电流过大导致小腿电机升温问题
- 修复 Roban2.2 多次切换 AMP/MPC 步态时插值异常导致摔倒，增加膝关节插值限位保护
- 修复 Kuavo5 PICO 遥操解锁手臂后头部跟踪抖动问题，调整启动方式防止参数错误
- 修改 VR 切换手臂控制模式逻辑以符合操作文档
- 将头部相机 camera 重命名为 head_camera_depth（原点为深度镜头），修复所有 gazebo URDF mesh 路径
- Roban 的 LB+A/LB+Y 不再触发步态缩放，修复步幅减小无限制导致机器飞出问题
- 更新轮臂BT2/BT2Pro手柄控制说明文档，补充启动命令和接收器安装步骤，[文档链接](./src/humanoid-wheel-control/humanoid_wheel_interface_ros/轮臂手柄控制说明.md)
- 补充H12/G12遥控器使用说明和手动启动注意事项，[文档链接](./src/humanoid-control/h12pro_controller_node/多控制器H12部署和操作说明.md)
- 修复手臂插值抖动相关问题：修复插值过程中时间轨迹判断跳过模式切换完成判断的bug，添加手臂模式重复切换防护，优化切换到外部控制模式时的抖动
- 修复 53、55 版本仿真 RL 控制器手臂控制异常
- 修复轮臂 G12 躯干控制向上超出限位和笛卡尔位置指令导致躯体抽搐的问题
- 修复 Roban2.2 VR 模式无法解锁手臂的问题（s17手臂STL文件路径错误）
- 修复 Roban手柄切换后映射错乱问题，恢复北通与Xbox手柄映射切换逻辑并在切换帧跳过旧映射处理
- 修复 v52 AMP 控制器切换时手臂关节增益缺失导致手臂乱甩
- 修复 VR 头部控制模式切换异常：跟随主动手模式切换到其他模式后复位再解锁手臂仍回到主动手模式
...
## 其他改进
- 解决鲁班机型手臂在行走中摆动不一致、不自然的问题
- 轮臂增加关节反馈异常检测，避免单关节反馈异常时影响整机扭矩输出（如搬箱子时机器后仰）
