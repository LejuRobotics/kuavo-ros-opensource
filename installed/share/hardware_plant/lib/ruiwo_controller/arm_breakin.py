...
print(f"{get_timestamp()} 等待腿部准备完成信号...")

# 等待腿部信号文件出现
while not os.path.exists(sync_signal_file):
    time.sleep(0.1)

print(f"{get_timestamp()} 收到腿部准备完成信号，开始同步...")
...
# 心跳文件路径
HEARTBEAT_FILE = "/tmp/arm_heartbeat"
...
def write_heartbeat():
    ...
    try:
        with open(HEARTBEAT_FILE, 'w') as f:
            ...
            f.write(f"arm_moving\n")
    except Exception as e:
        print(f"{get_timestamp()} 写入心跳文件失败: {e}")

# 信号处理函数
def signal_handler(signum, frame):
    """处理中断信号"""
    print(f"\n{get_timestamp()} 收到停止信号，正在安全停止磨线程序...")
    stop_program.set()
    # 创建停止信号文件，通知主程序停止
    try:
        ...
        print(f"{get_timestamp()} 创建停止信号文件失败: {e}")
...
print("\033[92m提示：按 Ctrl+C 可以立即停止磨线程序。\033[0m")
...
while True:
    # 检查是否收到停止信号
    if stop_program.is_set() or os.path.exists("/tmp/arm_stop_signal"):
        print(f"{get_timestamp()} 检测到停止信号，正在安全停止磨线程序...")
        break
...
try:
    ...
    print(f"{get_timestamp()} 清理停止信号文件失败: {e}")
