# general_motion_retargeting/utils/motivestreaming.py
#
# Build a calibration pose from OptiTrack / Motive rigid-body descriptions.
# Outputs a LAFAN-style pose dict (Hips/Spine/LeftArm/LeftForeArm/...) so that
# it matches the offline BVH loader naming convention.
#
# Author: you + ChatGPT (patch: Motive->LAFAN remap + robust A-pose arm rotation)
...
def remap_pose_names_motive_to_lafan(
    pose: Dict[str, Tuple[np.ndarray, np.ndarray]],
    mapping: Dict[str, str] = MOTIVE_TO_LAFAN,
...
def cali_pose_from_rb_desc_(
    ...
    pose = remap_pose_names_motive_to_lafan(pose_raw) if output_lafan_names else pose_raw
...
__all__ = [
    "MOTIVE_TO_LAFAN",
    "remap_pose_names_motive_to_lafan",
    "cali_pose_from_rb_desc_",
]
