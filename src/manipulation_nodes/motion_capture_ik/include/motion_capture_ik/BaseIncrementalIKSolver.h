...
class BaseIncrementalIKSolver {
 public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW

  explicit BaseIncrementalIKSolver(drake::multibody::MultibodyPlant<double>* plant,
                                   const std::vector<std::string>& ikConstraintFrameNames,
                                   const IKSolverConfig& config);
  virtual ~BaseIncrementalIKSolver() = default;
 ...
 protected:
  ...
  // common interface
  Eigen::VectorXd getWarmStartSolution(const Eigen::VectorXd& jointMidValues = Eigen::VectorXd(),
                                       ArmIdx controlArmIndex = ArmIdx::LEFT) const;
  ...
  bool preSolveCheck(const std::vector<PoseData>& PoseConstraintList,
                     const Eigen::VectorXd& jointMidValues = Eigen::VectorXd(),
                     int nq = -1,
                     ArmIdx controlArmIndex = ArmIdx::LEFT) const;
