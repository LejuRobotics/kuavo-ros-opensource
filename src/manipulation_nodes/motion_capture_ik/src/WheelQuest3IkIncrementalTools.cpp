...
void WheelQuest3IkIncrementalROS::reset() {
  ...
  // mode 1/2 切换时不能携带上一次增量模式的 grip 过渡状态。
  // 否则重新进入 mode 2 后，旧的释放快照会覆盖 mode 2 初始化的默认手部位置，
  // 使按下 grip 时手臂直接跳回上一次模式中的目标位置。
  leftGripTransferPending_ = false;
  rightGripTransferPending_ = false;
  leftGripOrientationHoldFrames_ = 0;
  rightGripOrientationHoldFrames_ = 0;
  leftGripTransferLockFrames_ = 0;
  rightGripTransferLockFrames_ = 0;
  leftGripTransferAccepted_ = false;
  rightGripTransferAccepted_ = false;
  hasPreviousLeftGripQuat_ = false;
  hasPreviousRightGripQuat_ = false;
  previousLeftGripQuat_.setIdentity();
  previousRightGripQuat_.setIdentity();
  hasLeftGripReleaseSnapshot_ = false;
  hasRightGripReleaseSnapshot_ = false;
  leftGripReleaseHandPos_.setZero();
  rightGripReleaseHandPos_.setZero();
  leftGripReleaseElbowPos_.setZero();
  rightGripReleaseElbowPos_.setZero();
  leftGripReleaseHandQuat_.setIdentity();
  rightGripReleaseHandQuat_.setIdentity();

  // 这些值会参与未激活手臂和躯干的冻结约束，也必须回到默认参考，
  // 避免 mode 1 中的躯干位置/高度被带入下一次 mode 2。
  frozenRobotChestPos_ = robotFixedWaistYawPos_;
  frozenChestQuat_.setIdentity();
  frozenLeftHandHeightOffset_ = 0.0;
  frozenRightHandHeightOffset_ = 0.0;
  frozenLeftElbowHeightOffset_ = 0.0;
  frozenRightElbowHeightOffset_ = 0.0;
  // 重置 grip 超时机制状态
  {
