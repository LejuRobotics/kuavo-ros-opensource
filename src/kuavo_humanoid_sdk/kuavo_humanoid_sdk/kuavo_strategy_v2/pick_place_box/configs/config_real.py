...
class config:
    class common:
        ...
        arm_pos_threshold = 0.35  # 手臂位置阈值，单位米
        arm_angle_threshold = np.deg2rad(25)  # 手臂角度阈值，单位弧度
    ...
    class place:
        ...
        torso_in_tag_pos = (0.0, 0.0, 1.22)  # 躯干在目标标签中的位置猜测，单位米
        torso_in_tag_euler = (-np.deg2rad(90), np.deg2rad(89), 0.0)  # 躯干在目标标签中的姿态猜测，单位欧拉角（弧度）

        box_behind_tag = 0.2  # 箱子在tag后面的距离，单位米
        box_beneath_tag = 0.4  # 箱子在tag下方的距离，单位米
