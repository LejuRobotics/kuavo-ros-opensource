...
class ArmAPI:
    ...
    def _move_joint_traj(self,
        ...
        PUBLISH_RATE = 100.0  # 固定下发频率 100Hz
        
        # 切换到外部控制模式
        self.robot_sdk.control.set_external_control_arm_mode()
        self.robot_sdk.control.set_manipulation_mpc_mode(KuavoManipulationMpcCtrlMode.ArmOnly)

        num_points_orig = len(joint_traj)
        if num_points_orig < 2:
            self.robot_sdk.control.control_arm_joint_positions(joint_positions=joint_traj[0])
            return
        
        # 根据总时间和固定频率计算需要的点数
        num_points_target = int(total_time * PUBLISH_RATE) + 1
        dt = 1.0 / PUBLISH_RATE  # 0.01s
        ...
        for t in t_target:
            ...
            if idx == 0:
                resampled_traj.append(joint_traj_np[0].tolist())
            elif idx >= num_points_orig:
                resampled_traj.append(joint_traj_np[-1].tolist())
            else:
                t0, t1 = t_orig[idx - 1], t_orig[idx]
                alpha = (t - t0) / (t1 - t0)
                interpolated = (1 - alpha) * joint_traj_np[idx - 1] + alpha * joint_traj_np[idx]
                resampled_traj.append(interpolated.tolist())

        # 以 100Hz 频率下发
        for i, joint_pos in enumerate(resampled_traj):
...
class TorsoAPI:
    ...
    def move_torso_pose(self,
        ...
        self._move_torso_pose(desir_torso_pose, control_base)
        return None
