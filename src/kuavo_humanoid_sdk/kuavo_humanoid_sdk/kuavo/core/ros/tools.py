...
class KuavoRobotToolsCore:
    ...
    Attributes:
        tf_buffer (tf2_ros.Buffer): TF2 buffer for storing transforms
        tf_listener (tf2_ros.TransformListener): TF2 listener for receiving transforms
        _transform_cache (dict): Cache for storing recent transforms
    """

    def __init__(self):
        """Initializes TF2 TransformListener for direct TF queries."""
        if not hasattr(self, '_initialized'):
            try:
                ...
                SDKLogger.debug("TF2 TransformListener initialized successfully")
            except Exception as e:
                SDKLogger.error(f"Failed to initialize TF2 TransformListener: {str(e)}")
                raise
    ...
    def _get_tf_tree_transform(self, target_frame: str, source_frame: str,
        ...
        """Gets transform between coordinate frames using tf2_ros TransformListener.
        ...
        try:
            ...
            try:
                ...
                SDKLogger.error(f"TF lookup failed: {str(e)}")
                SDKLogger.error(f"Transform from {source_frame} to {target_frame} not found")
                return None
            except tf2_ros.ConnectivityException as e:
                SDKLogger.error(f"TF connectivity error: {str(e)}")
                return None
            except tf2_ros.ExtrapolationException as e:
                SDKLogger.error(f"TF extrapolation error: {str(e)}")
                return None
