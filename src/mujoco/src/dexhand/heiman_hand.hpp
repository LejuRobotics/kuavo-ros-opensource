...
public:
    ...
    // Start by holding the simulated joint positions.  A default hardware
    // rest pose must not move either hand before the first SG100 command.
    // Afterwards, filter real commands at 50 Hz with alpha=0.2.
    void writeCallback(mjData *d) override {
        ...
        const bool first_write = !filter_initialized_;
        const bool time_reset =
            filter_initialized_ && d->time + 1e-9 < last_filter_time_;
        const bool reset = first_write || time_reset;
        if (reset) {
            const bool command_arrived_before_first_write =
                first_write && ctrl_updated_.exchange(false);
            int i = 0;
            for (auto iter = jga_.qposadr().begin();
                 iter != jga_.qposadr().end() && i < JOINT_COUNT; ++iter, ++i) {
                filtered_positions_[i] = d->qpos[*iter];
                if (!command_arrived_before_first_write) {
                    target_positions_[i] = d->qpos[*iter];
                }
            }
