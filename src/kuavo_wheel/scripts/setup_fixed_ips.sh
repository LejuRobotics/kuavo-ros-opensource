...
get_interface_ip() {
    ...
    if command -v ifconfig >/dev/null 2>&1; then
        ip_info=$(ifconfig "$interface" 2>/dev/null | grep "inet " | awk '{print $2}' | head -1)
    elif command -v ip >/dev/null 2>&1; then
        ip_info=$(ip addr show "$interface" 2>/dev/null | grep "inet " | awk '{print $2}' | cut -d'/' -f1 | head -1)
    fi
...
select_network_interface() {
    ...
    # 检查是否有且仅有一个169开头的接口
    local matched_169=($(find_169_interfaces))
    
    if [ ${#matched_169[@]} -eq 1 ]; then
        log_info "自动检测到1个169开头的网卡: ${matched_169[0]}"
        local auto_ip=$(get_interface_ip "${matched_169[0]}")
        if [ -n "$auto_ip" ]; then
            log_info "当前IP: $auto_ip"
        fi
        echo "${matched_169[0]}"
        return 0
    elif [ ${#matched_169[@]} -gt 1 ]; then
        log_warn "检测到 ${#matched_169[@]} 个169开头的网卡，需要手动选择"
    fi
...
configure_static_ip() {
    ...
    # 如果 NetworkManager 正在运行，先禁用对该接口的管理
    if command -v nmcli >/dev/null 2>&1 && systemctl is-active --quiet NetworkManager 2>/dev/null; then
        log_info "检测到 NetworkManager，禁用对接口 $interface 的自动管理..."
...
"
    ...
    # 如果 NetworkManager 正在运行，先禁用对该接口的管理
    if command -v nmcli >/dev/null 2>&1 && systemctl is-active --quiet NetworkManager 2>/dev/null; then
        log_info "禁用 NetworkManager 对接口 $interface 的管理..."
        sudo nmcli device set "$interface" managed no 2>/dev/null || true
    fi
    
    # 确保 systemd-networkd 服务正在运行（如果使用 networkd renderer）
    if systemctl list-unit-files | grep -q systemd-networkd.service; then
        if ! systemctl is-active --quiet systemd-networkd 2>/dev/null; then
            log_info "启动 systemd-networkd 服务..."
