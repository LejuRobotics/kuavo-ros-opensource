...
class NatNetClient:
    ...
    def __init__(self):
        ...
        # Callback when Data Descriptions (model def) are received from server.
    ...
    def __create_data_socket(self):
        ...
        if self.use_multicast:
            ...
            # Unicast case: 绑定 data_port(1511)，Motive 默认向客户端 1511 发帧数据
            ...
            try:
                ...
                print(f'Unicast Data Socket Error (port %d): {e}' % self.data_port)
                sys.exit(1)
        return result
...
# Unpack Mocap Data Functions
    ...
    def __command_thread_function(self, in_socket, stop, gprint_level, thread_option): #type: ignore  # noqa E501
        ...
        while not stop():
            ...
            except socket.error: #type: ignore  # noqa F841
                if stop():
                    print("shutting down")
                    return 0
    ...
    def __data_thread_function(self, in_socket, stop, gprint_level):
        ...
        while not stop():
            try:
                ...
                if errno == 9:  # EBADF: socket 已被关闭，多为正常退出时竞态
                    return 0
                print("ERROR: data socket 异常 (errno=%s)，可能 Motive 断开或网络波动" % errno)
                return 1
            except socket.herror:
    ...
    def run(self, thread_option):
        ...
        # Create a separate thread for receiving data packets (daemon so process can exit without waiting)
        ...
        if thread_option == 'd':
            ...
            cmd_addr = self.command_socket.getsockname()
    ...
    def shutdown(self):
        ...
        # attempt to join the threads back (timeout to avoid blocking forever)
        join_timeout = 2.0
        if self.command_thread.is_alive():
            self.command_thread.join(timeout=join_timeout)
        if self.data_thread.is_alive():
            self.data_thread.join(timeout=join_timeout)
