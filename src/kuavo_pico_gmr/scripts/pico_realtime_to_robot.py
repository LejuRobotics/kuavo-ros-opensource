...
from kuavo_msgs.msg import (
    ControllerSwitchEvent, JoySticks, picoPoseInfoList, picoPoseRetarget
)
...
class PicoStreamingNode:
    ...
    def __init__(self):
        ...
        self.vmp_yaw_resume_ready_pub = rospy.Publisher(
            '/pico/vmp_yaw_resume_ready', Header, queue_size=1
        )
    ...
    def publish_vmp_input(self, qpos_raw, root_loc, root_rot_wxyz, dof_pos, dt_sec, stamp=None):
        ...
        if self.arm_traj_pub is not None:
            if len(dof_pos) < 27 or len(dof_vel) < 27:
                rospy.logwarn_throttle(
                    1.0,
                    f"[PICO] Cannot publish /kuavo_arm_traj: expected 27 DOF, "
                ...
                arm_traj.position = np.rad2deg(np.asarray(dof_pos[13:27], dtype=np.float64)).tolist()
                arm_traj.velocity = np.rad2deg(np.asarray(dof_vel[13:27], dtype=np.float64)).tolist()
