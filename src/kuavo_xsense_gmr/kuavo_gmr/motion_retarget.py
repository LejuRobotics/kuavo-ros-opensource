...
def _qmul_wxyz(q1, q2):
    """Hamilton product of two quaternions in (w,x,y,z) order. Pure numpy."""
...
def _qnorm_wxyz(q):
    """Normalize quaternion (w,x,y,z), enforce w>=0."""
...
class GeneralMotionRetargeting:
    ...
    def setup_retarget_configuration(self):
        ...
        # Pre-cache numpy data for fast offset_human_data (avoid per-frame scipy)
        self._build_offset_cache()

    def _build_offset_cache(self):
        """Pre-compute rot_offset quaternions and matrices for numpy-only offset_human_data."""
        self._rot_offsets1_quat = {}   # body_name -> (4,) ndarray wxyz
        self._rot_offsets1_mat = {}    # body_name -> (3,3) ndarray
        self._pos_offsets1_arr = {}    # body_name -> (3,) ndarray
        for body_name, rot_obj in self.rot_offsets1.items():
            q_wxyz = rot_obj.as_quat(scalar_first=True).astype(np.float64)
            self._rot_offsets1_quat[body_name] = q_wxyz
            self._rot_offsets1_mat[body_name] = rot_obj.as_matrix().astype(np.float64)
            self._pos_offsets1_arr[body_name] = np.asarray(self.pos_offsets1[body_name], dtype=np.float64)

        self._rot_offsets2_quat = {}
        self._rot_offsets2_mat = {}
        self._pos_offsets2_arr = {}
        for body_name, rot_obj in self.rot_offsets2.items():
            q_wxyz = rot_obj.as_quat(scalar_first=True).astype(np.float64)
            self._rot_offsets2_quat[body_name] = q_wxyz
            self._rot_offsets2_mat[body_name] = rot_obj.as_matrix().astype(np.float64)
    ...
    def offset_human_data(self, human_data, pos_offsets, rot_offsets):
        ...
        if rot_offsets is self.rot_offsets1:
            quat_cache = self._rot_offsets1_quat
            pos_cache = self._pos_offsets1_arr
        else:
            quat_cache = self._rot_offsets2_quat
        ...
        for body_name in human_data.keys():
            ...
            if body_name not in quat_cache:
                offset_human_data[body_name] = [pos, quat]
                continue

            # updated_quat = quat * rot_offset  (Hamilton product, wxyz)
            updated_quat = _qmul_wxyz(quat, quat_cache[body_name])
            updated_quat = _qnorm_wxyz(updated_quat)

            # global_pos_offset = R(updated_quat) @ local_offset
            R_mat = _quat_wxyz_to_mat(updated_quat)
            global_pos_offset = R_mat @ pos_cache[body_name]
    ...
    def apply_ground_offset(self, human_data):
        go = self.ground_offset
        if go == 0.0:
            return human_data
        offset_vec = np.array([0.0, 0.0, go], dtype=np.float64)
        for body_name in human_data.keys():
            pos, quat = human_data[body_name]
            human_data[body_name][0] = pos - offset_vec
        return human_data
