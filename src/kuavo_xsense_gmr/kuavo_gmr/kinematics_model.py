"""
Kinematics model for the Mujoco model
Not used in the optimisation process, only used for visualizing the retargetd results
"""
...
class KinematicsModel:
    ...
    def compute_end_effector_positions(self, body_positions, body_rotations, end_effector_config):
        """
        Compute end effector positions given body positions/rotations and offset configurations.
        
        Args:
            body_positions: Tensor of shape [batch_size, num_joints, 3] or [num_joints, 3]
            body_rotations: Tensor of shape [batch_size, num_joints, 4] or [num_joints, 4] (quaternions)
            end_effector_config: Dict mapping end effector names to (body_name, local_offset)
                Example: {
                    "left_toe_tip": ("leg_l6_link", [0.0, 0.0, -0.05]),
                    "right_hand_palm": ("arm_r7_link", [0.0, 0.0, 0.1])
                }
        
        Returns:
            Dictionary mapping end effector names to positions (tensors)
        """
        end_effector_positions = {}
        
        for ee_name, (body_name, local_offset) in end_effector_config.items():
            # Get body index
            body_idx = self.get_body_idx(body_name)
            # Get body position and rotation
            if len(body_positions.shape) == 3:
                body_pos = body_positions[:, body_idx, :]
                body_rot = body_rotations[:, body_idx, :]
            else:
                body_pos = body_positions[body_idx, :]
                body_rot = body_rotations[body_idx, :]
            # Convert local offset to tensor
            offset_tensor = torch.tensor(local_offset, dtype=body_pos.dtype, device=body_pos.device)
            # Broadcast offset to match batch dimension if needed
            if len(body_positions.shape) == 3:
                offset_tensor = torch.broadcast_to(offset_tensor, body_pos.shape)
            # Rotate the local offset to world frame
            world_offset = torch_utils.quat_rotate(body_rot, offset_tensor)
            # Compute end effector position
            ee_pos = body_pos + world_offset
            end_effector_positions[ee_name] = ee_pos
