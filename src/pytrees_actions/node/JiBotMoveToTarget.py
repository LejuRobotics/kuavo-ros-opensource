...
class JiBotMoveToTarget(Behaviour):
    def __init__(self, name: str, label: str, namespace: str, params):
        super(JiBotMoveToTarget, self).__init__(name)
        self.params = params
        self.label = label.split('/', -1)[-1]

        # 服务名称
        self.service_name = self.params.get('service_name', '/5w_move_to_target')
        ...
        self.logger.info(f"JiBotMoveToTarget initialized (参数将在 initialise 中验证)")
    ...
    def initialise(self):
        ...
        try:
            ...
            self.service_proxy = rospy.ServiceProxy(self.service_name, MoveToTarget)
    ...
    def terminate(self, new_status):
        """节点终止时的清理工作"""
        self.logger.info(f"JiBotMoveToTarget 节点终止，状态: {new_status}")
