...
class PrepareCalcLeg(Behaviour):
    """
    ...
    def initialise(self):
        ...
        if self.leg_mode == "target":
            try:
                ...
                stand_pose_in_tag_future=Pose.from_euler(
                ...
                stand_pose_in_world_future = self.percep_event.transform_pose_from_tag_to_world(target_tag, stand_pose_in_tag_future)

                xy_offset = [
                    stand_pose_in_world_future.pos[0] - current_pos[0],  # X差异
                    stand_pose_in_world_future.pos[1] - current_pos[1],  # Y差异
                    stand_pose_in_world_future.pos[2] - current_pos[2]   # Z差异
                ]
                ...
                stand_pose_in_world = Pose(
                    pos=[
                    stand_pose_in_world.pos[0] - xy_offset[0],  # 未来躯干X - X差异
                    stand_pose_in_world.pos[1] - xy_offset[1],  # 未来躯干Y - Y差异
                    stand_pose_in_world.pos[2]   # Z保持不变
                    ],
                    quat=stand_pose_in_world.quat,  # 保持原来的姿态
                    frame=Frame.ODOM
                )
