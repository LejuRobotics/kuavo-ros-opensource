                                  ...
                                  const sensor_msgs::JointState& state)
{
  ...
  // 可选调试：如需查看 FK 结果，可以临时取消下面注释。
  // static int fk_debug_calls = 0;
  // if ((name_ == "head_chain" || name_ == "head_camera") && fk_debug_calls < 1)
  // {
  //   double roll, pitch, yaw;
  //   p_out.M.GetRPY(roll, pitch, yaw);
  //   ROS_INFO_STREAM("FK(" << name_ << ") root=" << root_
  //                   << " tip=" << tip_
  //                   << " p=[" << p_out.p.x() << ", "
  //                   << p_out.p.y() << ", "
  //                   << p_out.p.z() << "]"
    ...
    const CalibrationOffsetParser& offsets)
{
  ...
  for (size_t i = 0; i < points.size(); ++i)
  {
    ...
    // 调试：只在前几次调用时打印第 0 个点的原始相机坐标
    static int cam_debug_calls = 0;
    if (i == 0 && cam_debug_calls < 5)
    {
      ROS_INFO_STREAM("Camera3dModel raw feature[0] in sensor frame '"
                      << data.observations[sensor_idx].features[i].header.frame_id
                      << "' for sensor '" << name_ << "': "
                      << "p=[" << x << ", " << y << ", " << z << "]");
      cam_debug_calls++;
    }
