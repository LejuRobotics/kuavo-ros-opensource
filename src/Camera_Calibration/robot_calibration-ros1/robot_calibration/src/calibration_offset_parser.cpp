  ...
  for (TiXmlElement* joint_xml = robot_xml->FirstChildElement("joint"); joint_xml; joint_xml = joint_xml->NextSiblingElement("joint"))
  {
    ...
    if (has_update)
    {
      ...
      if (origin_xml)
      {
        ...
        // token_compress_on: URDF 中 rpy/xyz 偶发双空格，否则 split 会得到空串导致 size!=3、旋转保持单位阵
        boost::split(xyz_pieces, xyz_str, boost::is_any_of(" "), boost::token_compress_on);

        // Split out rpy of origin, break into 3 strings
        std::vector<std::string> rpy_pieces;
        boost::split(rpy_pieces, rpy_str, boost::is_any_of(" "), boost::token_compress_on);
