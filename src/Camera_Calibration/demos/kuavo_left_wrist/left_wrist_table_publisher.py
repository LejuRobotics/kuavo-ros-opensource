...
def _find_latest_teach_json(teach_dir: Path) -> Optional[Path]:
    patterns = ["teach_left_joint*.json", "teach_joints_*.json"]
    all_files: List[Path] = []
    for pat in patterns:
...
def load_left_arm_points_from_teach_json(json_path: Path) -> List[Dict]:
    ...
    for s in samples:
        if not isinstance(s, dict):
            continue
        if "left_arm_joints" not in s:
            continue
        arr = s["left_arm_joints"]
        if not isinstance(arr, list) or len(arr) == 0:
            continue
        idx = s.get("index", len(points))
        q_deg = [_deg(float(x)) for x in arr]
        points.append({"name": f"T{idx}", "left_deg": q_deg})

    if not points:
        raise ValueError(f"teach json 未找到 left_arm_joints: {json_path}")
    return points
...
class SensorsArmMonitor:
    ...
    get_current_left_right_deg() 返回度；发布到 /kuavo_arm_traj 的 JointState.position 也必须是度，
...
def publish_segment(
    pub: rospy.Publisher,
    start_left_deg: List[float],
    end_left_deg: List[float],
    right_hold_deg: List[float],
    duration: float,
    dt: float,
) -> None:
    ...
    if duration <= 1e-9:
        ...
        js.position = [float(x) for x in end_left_deg] + [float(x) for x in right_hold_deg]
        js.velocity = [0.0] * 14
        js.effort = [0.0] * 14
        pub.publish(js)
        return
    ...
    while not rospy.is_shutdown():
        ...
        q = [interp(start_left_deg[i], end_left_deg[i], ratio) for i in range(7)]
        js = JointState()
        js.header.stamp = rospy.Time.now()
        js.name = LEFT_ARM_JOINT_NAMES + RIGHT_ARM_JOINT_NAMES
        js.position = [float(x) for x in q] + [float(x) for x in right_hold_deg]
        js.velocity = [0.0] * 14
        js.effort = [0.0] * 14
        pub.publish(js)
        rate.sleep()
...
def hold_and_capture(
    ...
    while not rospy.is_shutdown():
        ...
        js.header.stamp = rospy.Time.now()
        js.name = LEFT_ARM_JOINT_NAMES + RIGHT_ARM_JOINT_NAMES
        js.position = [float(x) for x in left_deg] + [float(x) for x in right_hold_deg]
        js.velocity = [0.0] * 14
        js.effort = [0.0] * 14
        pub.publish(js)
...
def main() -> None:
    rospy.init_node("left_wrist_table_publisher", argv=sys.argv, anonymous=False)
    ...
    def _try_call(service_name: str, mode: int) -> bool:
        try:
            ...
            if hasattr(resp, "result") and resp.result:
                ...
                return True
            rospy.logwarn(
                "Service %s failed: mode=%s msg=%s",
                service_name,
                getattr(resp, "mode", None),
    ...
    if teach_json_path_param:
        ...
        teach_json_path = teach_output_dir / "teach_left_joint.json"
    if not teach_json_path.is_file():
        raise RuntimeError(f"找不到 teach json: {teach_json_path}（或请设置 ~teach_json_path）")

    points = load_left_arm_points_from_teach_json(teach_json_path)
    ...
    # 切模式后立即用缓存的当前角做预热保持，再从当前角插值到第一帧
    warmup_hold_sec = float(rospy.get_param("~warmup_hold_sec", 0.8))
    rospy.loginfo(
        "起步预热保持: %.2fs, current_left=%s, first_target=%s",
        warmup_hold_sec,
        [round(x, 2) for x in current_left],
        [round(x, 2) for x in points[0]["left_deg"]],
    )
    hold_pose(arm_pub, current_left, right_hold, warmup_hold_sec, dt)

    loops = max(1, play_loop_count)
    rospy.loginfo(
        "启动 left_wrist_table_publisher: loops=%d, move=%.2fs, hold=%.2fs, capture_at=%.2fs, points=%d",
        loops,
        move_duration,
        hold_sec,
        capture_at_sec,
    ...
    for loop_idx in range(loops):
        ...
        first = points[0]
        target_left = first["left_deg"]
        publish_segment(arm_pub, current_left, target_left, right_hold, startup_align_sec, dt)
        current_left = list(target_left)
        # 左右手首尾关键帧通常难以稳定识别棋盘：跳过第 1 帧与最后一帧的采样触发
        # 关键帧触发改为仅在后续循环中对 k=2..N-1 触发（并压缩编号为 1..N-2）

        for i in range(len(points) - 1):
            a = points[i]["left_deg"]
            b = points[i + 1]["left_deg"]
            publish_segment(arm_pub, a, b, right_hold, move_duration, dt)
            current_left = list(b)
            # 关键帧原始序号 k = i+2（1-based）；跳过 k=1 与 k=N，保留 k=2..N-1
            k = i + 2
            n_total = len(points)
    ...
    # 采样完成后，左臂从当前位姿平滑回零位。
    zero_left = [0.0] * 7
    publish_segment(arm_pub, current_left, zero_left, right_hold, return_to_zero_sec, dt)
    current_left = list(zero_left)
    rospy.loginfo("左臂已回零位")

    done_msg = Bool()
    done_msg.data = True
    done_pub.publish(done_msg)
    rospy.loginfo("左手轨迹下发完成，已发送 done 标志")
