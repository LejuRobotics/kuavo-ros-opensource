...
- `src/Camera_Calibration/demos/kuavo_left_wrist/kuavo_left_wrist_demo.launch`

它是整个链路里的 “左腕相机” 部分：负责把 **左腕相机的棋盘角点观测** 与 **机器人关节角** 采集成 CSV，并调用优化器输出 `calibration.yaml`。
...
- 当前在线话题对齐（2D 棋盘角点）：
  ...
  - `frame_id="left_wrist_camera_color_optical_frame"`

### 从优化结果写入零点

左腕 demo 的 `calibration.yaml` 会被顶层脚本统一合并并写入零点文件：
