...
class TrajectoryController:
    ...
    def __init__(self, robot, publish_frequency=100.0,
        ...
        Args:
            ...
            initial_positions (List[float], optional): 14 个实测关节弧度。
                省略时保留原有全零起点行为。
        """
        self.robot = robot
        ...
        if initial_positions is not None and len(initial_positions) != 14:
            raise ValueError(
                "Expected 14 initial joint positions, got {}".format(
                    len(initial_positions)))

        # 当前目标关节角度。新任务可从实测姿态开始，既有 task1 调用不变。
        self.current_target_positions = (
            list(initial_positions)
            if initial_positions is not None else [0.0] * 14)
