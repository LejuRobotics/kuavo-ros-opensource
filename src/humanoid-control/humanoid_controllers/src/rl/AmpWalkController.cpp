...
namespace humanoid_controller
{
  ...
  bool AmpWalkController::loadConfig(const std::string& rlParamFile)
  {
    ...
    std::string obs_stack_order = "classic";
    loadData::loadPtreeValue(pt, obs_stack_order, "obsStackOrder", false);
    use_isaaclab_stack_order_ = (obs_stack_order == "isaaclab");
                                            ...
                                            const SensorData& sensor_data)
  {
    ...
    if (use_isaaclab_stack_order_)
    {
      int key_offset = 0;
      int output_offset = 0;
      for (const auto &key : singleInputDataKeys_)
      {
        const int width = static_cast<int>(singleInputDataID_[key][1]);
        for (int i = 0; i < frameStack_; ++i)
        {
          networkInputDataRL_.segment(output_offset, width) =
              inputDeque_[i].segment(key_offset, width);
          output_offset += width;
        }
        key_offset += width;
      }
