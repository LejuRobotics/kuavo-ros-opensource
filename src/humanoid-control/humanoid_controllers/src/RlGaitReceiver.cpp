  ...
  , velocity_smooth_time_(0.1)
{
  ...
  // Initialize YAW compensation parameters from ROS parameter server
  nh_.param<bool>("/yawCompensationEnabled", yaw_compensation_enabled_, true);
  nh_.param<double>("/yawThreshold", yaw_threshold_, 0.05);
  nh_.param<double>("/yawXVelocityThreshold", yaw_x_velocity_threshold_, 0.01);
  nh_.param<bool>("/enableSeparateYawCompensation", enable_separate_yaw_compensation_, true);
  nh_.param<double>("/yawXBiasClockwise", yaw_x_bias_clockwise_, 0.004);
  nh_.param<double>("/yawXBiasCounterclockwise", yaw_x_bias_counterclockwise_, 0.0);
  yaw_x_bias_ = 0.0;  // 保留用于非分离模式
  
  ROS_INFO("[RlGaitReceiver] YAW补偿参数加载完成:");
  ROS_INFO("  - 启用状态: %s", yaw_compensation_enabled_ ? "true" : "false");
  ROS_INFO("  - YAW阈值: %.4f", yaw_threshold_);
  ROS_INFO("  - X速度阈值: %.4f", yaw_x_velocity_threshold_);
  ROS_INFO("  - 分离补偿: %s", enable_separate_yaw_compensation_ ? "true" : "false");
  ROS_INFO("  - 顺时针补偿: %.4f", yaw_x_bias_clockwise_);
  ROS_INFO("  - 逆时针补偿: %.4f", yaw_x_bias_counterclockwise_);
...
void RlGaitReceiver::cmdVelCallback(const geometry_msgs::Twist::ConstPtr& msg)
{
  ...
  smoothed_vel = applyYawCompensation(smoothed_vel);
...
geometry_msgs::Twist RlGaitReceiver::applyYawCompensation(const geometry_msgs::Twist& input_vel)
{
  geometry_msgs::Twist compensated_vel = input_vel;
  ...
  // 如果X方向已经有速度，则不进行YAW补偿
  if (std::abs(input_vel.linear.x) > yaw_x_velocity_threshold_)
  {
    return compensated_vel;
  }
  
  // 检查YAW角速度是否超过阈值
  if (std::abs(input_vel.angular.z) > yaw_threshold_)
  {
    if (enable_separate_yaw_compensation_)
    {
      // 顺时针旋转（角速度为正）
      if (input_vel.angular.z > 0)
      {
        compensated_vel.linear.x += yaw_x_bias_clockwise_;
      }
      // 逆时针旋转（角速度为负）
      else
      {
        compensated_vel.linear.x += yaw_x_bias_counterclockwise_;
      }
    }
    else
    {
      // 使用统一的YAW补偿
      double yaw_sign = (input_vel.angular.z > 0) ? 1.0 : -1.0;
      compensated_vel.linear.x += yaw_x_bias_ * yaw_sign;
    }
    
    ROS_DEBUG("[RlGaitReceiver][YawCompensation] YAW角速度: %.3f, X方向速度: %.3f, 补偿后X轴速度: %.3f",
              input_vel.angular.z, input_vel.linear.x, compensated_vel.linear.x);
  }
