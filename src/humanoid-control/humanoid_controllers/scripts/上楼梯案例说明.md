...
- 前提条件：
  - 建议运行时先确认深度历史话题 `/camera/depth/depth_history_array` 已正常发布且频率稳定在 50Hz 左右；底层 `RLControllerManager` 会在切换前统一检查该话题，不满足条件时拒绝切换
  - 机器人当前处于 `stance`
- 触发按键：
...
4. 按 `E_RIGHT + F_LEFT + C_LONG_PRESS`
5. 系统先检查当前控制器是否允许切换
6. 底层 `RLControllerManager` 会统一检查深度历史话题是否满足切换条件
7. 如果检查通过，则切到 `depth_loco_controller`
