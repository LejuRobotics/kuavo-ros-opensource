...
# G11 全通道(含实体 SBUS 与虚拟 CH12~16)接收值域硬边界:
#   任何通道不可能出现 < 282 或 > 1722 的数值; 超范围视为噪声整帧忽略。
...
CH14_RECV_BASE = 1162
CH14_FORCE_BIT_OFFSET = 8     # force 位偏移(1170/1186)
CH14_CLAW_BIT_OFFSET = 16     # claw  位偏移(1178/1186)

def decode_ch14(ch14: int):
    """解码 CH14 扩展状态 → (force: bool, claw: bool)。非本值域(不在4档内)返回 None。"""
    offset = ch14 - CH14_RECV_BASE
    if offset not in (0, CH14_FORCE_BIT_OFFSET, CH14_CLAW_BIT_OFFSET,
                      CH14_FORCE_BIT_OFFSET + CH14_CLAW_BIT_OFFSET):
        return None
    force = bool(offset & CH14_FORCE_BIT_OFFSET)
    claw = bool(offset & CH14_CLAW_BIT_OFFSET)
    return force, claw
...
#         "skip"    = 暂不处理(仅日志)
#
# 屏幕菜单重构(2026-09): CH13 显式表, 不再用算术公式推导。
#   接收值 = (发送值 - 1050) * 1.6 + 282
SCREEN_CMD_MAP = {
    # --- 硬件启动 (人型/轮臂二级菜单 CH12=1010/1018) ---
...
class ScreenCmdParser:
    ...
    输入一帧的虚拟通道值(与物理通道号一致, 均为接收值):
      - CH12: 页面码(持续保持)
      - CH13: 功能码(脉冲, 上升沿触发)
      - CH14: 扩展状态通道(持续保持): 双 bit 组合编码
              force位(1170/1186)=启动前必须选类型; claw位(1178/1186)=旋钮控制夹爪
    """

    def __init__(self):
        self._prev_ch13 = PAGE_IDLE_RECV
        self._prev_valid = False
        self.boot_force = False   # CH14 force 位: 启动前必须选择机器人类型
        self.claw_mode = False    # CH14 claw  位: 旋钮控制夹爪末端跟随
        # 诊断计数(只累加, 不随 reset 清零): 供调用方节流上报,
        # 便于现场排查"偶尔不响应/丢一次指令"(屏幕端 README 要求越界帧丢弃并计数)
        self.noise_frames = 0     # 值域越界被丢弃的帧数
    ...
    def update(self, ch12_page: int, ch13_cmd: int, ch14: int = None):
        """喂入一帧 CH12/CH13(/CH14) 接收值。

        Args:
            ch12_page: CH12 页面码(接收值)
            ch13_cmd:  CH13 功能码(接收值)
            ch14:      CH14 扩展状态值(接收值, 可选); 语义由协议表统一解析

        Returns:
            dict | None: 命中时返回 {"func_name","type","target","cmd","page"}
            未触发(非上升沿/未知码/页面不匹配/值域越界噪声)返回 None。
        """
        # 值域校验: G11 全通道恒在 [282, 1722], 任一通道越界即整帧噪声, 直接
        # 忽略(不更新内部上升沿/页面状态, 避免噪声帧污染边沿检测);
        # 同时计数, 供调用方节流上报(屏幕端固件有偶发写 2100 -> 收 1962 的已知缺陷)
        if (not (CHANNEL_MIN_RECV <= ch12_page <= CHANNEL_MAX_RECV)
                or not (CHANNEL_MIN_RECV <= ch13_cmd <= CHANNEL_MAX_RECV)
                or (ch14 is not None
                    and not (CHANNEL_MIN_RECV <= ch14 <= CHANNEL_MAX_RECV))):
            self.noise_frames += 1
            return None

        # CH14 扩展状态解析(独立于 CH13 上升沿, 持续保持值双 bit 解码)
        if ch14 is not None:
            decoded = decode_ch14(ch14)
            if decoded is not None:
                force, claw = decoded
                self.boot_force = force
                self.claw_mode = claw

        rising = (ch13_cmd != PAGE_IDLE_RECV and
                  (not self._prev_valid or self._prev_ch13 == PAGE_IDLE_RECV))
        self._prev_ch13 = ch13_cmd
        self._prev_valid = True

        if not rising:
            return None
        if ch13_cmd not in SCREEN_CMD_MAP:
            if ch13_cmd in HIDDEN_CMDS:
        ...
        func_name, ftype, target = SCREEN_CMD_MAP[ch13_cmd]

        # 页面二次校验
        expect_page = page_of_cmd(ch13_cmd)
        if ftype == "boot":
            # 硬件启动码: 允许在人型(1010)/轮臂(1018) 二级菜单下发
            if ch12_page not in (PAGE_HUMANOID_RECV, PAGE_WHEEL_RECV):
                return {
                    "func_name": func_name, "type": ftype, "target": target,
                    "cmd": ch13_cmd, "page": ch12_page,
                    "rejected": True,
                    "expect_page": f"{PAGE_HUMANOID_RECV}/{PAGE_WHEEL_RECV}",
                }
        elif expect_page is not None and ch12_page != expect_page:
            return {
                "func_name": func_name, "type": ftype, "target": target,
                "cmd": ch13_cmd, "page": ch12_page,
        ...
        return {
            "func_name": func_name, "type": ftype, "target": target,
            "cmd": ch13_cmd, "page": ch12_page, "rejected": False,
        }
