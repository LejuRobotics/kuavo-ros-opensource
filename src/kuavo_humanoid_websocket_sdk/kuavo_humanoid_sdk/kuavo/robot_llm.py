...
class KuavoRobotLLM:
    ...
    def __init__(self):
        ...
        self._play_status_sub = roslibpy.Topic(
            self._ws_client, '/audio_status', 'kuavo_msgs/AudioPlaybackStatus'
        )
        self._play_status_sub.subscribe(self._play_status_callback)

        package_name = "planarmwebsocketservice"
        self.package_path = os.environ.get(
    ...
    def _play_status_callback(self, msg):
        """播放状态回调函数"""
        self.playing_status = msg.get('playing', False)
...
内容{self.knowledge_texts.index(text)+1}结束\n"
    ...
    def trigger_asr(self) -> str:
        ...
        # 启动音频接收和发送线程
        self._audio_receiver_sub = roslibpy.Topic(
            self._ws_client, '/micphone_data', 'kuavo_msgs/AudioReceiverData'
        )
        self._audio_receiver_sub.subscribe(self._audio_receiver_callback)
        print("订阅成功")
        ...
        # ASR完成后停止音频接收和发送
        self._audio_receiver_sub.unsubscribe()
        self._send_asr_audio_running = False
        # self.asr_end.close()
    ...
    def _audio_receiver_callback(self, msg):
        """麦克风数据回调函数"""
        try:
            # 播放状态：直接使用真实音频数据
            audio_data = bytes(msg.get('data', []))
    ...
    def _put_audio_into_ros(self):
        ...
        return:
            None
        """
        self.tts_audio_pub = roslibpy.Topic(
            self._ws_client, '/audio_data', 'std_msgs/Int16MultiArray'
        )
        while True:
            try:
                # 检查是否需要停止
                ...
                self.tts_audio_pub.publish(roslibpy.Message({"data": list(audio_data)}))
            except queue.Empty:
                time.sleep(0.01)
        print("【音频发布】线程已停止")
