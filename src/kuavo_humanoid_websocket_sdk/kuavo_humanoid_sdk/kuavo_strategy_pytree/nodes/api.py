...
class ChassisApi:
    ...
    def set_walk_goal_from_tag(
        ...
        if tag is None:
            ros_env.logwarn(f"❌ 黑板上没有找到 tag {tag_id}")
            return False
    ...
    def set_chassis_keypoints_from_walk_goal(self, chassis_time: float = 7.0) -> bool:
        ...
        if walk_goal is None:
            ros_env.logerr("ChassisApi: 黑板无 walk_goal")
            return False
        ...
        ros_env.loginfo(f"📌 底盘关键点: [x={x:.3f}, y={y:.3f}, yaw={yaw:.3f}], time={chassis_time}s")
        return True
...
class ArmAPI:
    ...
    def send_timed_arm_cmd(self, left_pose: Pose, right_pose: Pose, desire_time: float) -> Tuple[bool, float]:
        ...
        if desire_time <= 0:
            ros_env.logwarn(f"期望执行时间应大于0，当前值: {desire_time:.3f}")
        
        try:
            ros_env.wait_for_service('/mobile_manipulator_timed_single_cmd', timeout=5.0)
            client = ros_env.ServiceProxy('/mobile_manipulator_timed_single_cmd', lbTimedPosCmd)
            ...
            if ok_left and ok_right:
                ros_env.loginfo(f"手臂指令执行成功，实际时间: {actual_time}s")
            else:
                ros_env.logerr(f"手臂指令执行失败（左: {ok_left}, 右: {ok_right}）")
            return ok_left and ok_right, actual_time
        except ros_env.ROSException as e:
            ros_env.logerr(f"服务等待超时: {e}")
            return False, 0.0
        except ros_env.ServiceException as e:
            ros_env.logerr(f"服务调用失败: {e}")
            return False, 0.0
        except Exception as e:
            ros_env.logerr(f"未知错误: {e}")
            return False, 0.0
...
class TimedCmdAPI:
    ...
    def _call_timed_single_cmd(self, planner_index: int, cmd_vec: List[float],
        ...
        try:
            ...
            ros_env.logerr(f"单次指令失败 (planner={planner_index}): {resp.message}")
    ...
    def send_timed_cmd(self, cmd_type: str, cmd_vec: List[float], 
        ...
        if cmd_type not in self.CMD_CONFIG:
            ...
            ros_env.logerr(f"无效的命令类型: {cmd_type}，支持: {supported}")
            return False, 0.0
        ...
        if len(cmd_vec) != expected_dim:
            ros_env.logerr(f"{cmd_name}命令维度错误，需要{expected_dim}个，实际{len(cmd_vec)}个")
            return False, 0.0
        ...
        if 'split_left_planner' in config:
            ...
            if ok_left:
                ros_env.loginfo(f"{cmd_name}左臂指令执行成功，实际时间: {actual_left}s")
            if ok_right:
                ros_env.loginfo(f"{cmd_name}右臂指令执行成功，实际时间: {actual_right}s")
            success = ok_left and ok_right
            actual_time = max(actual_left, actual_right)
            if success:
                ros_env.loginfo(f"{cmd_name}指令执行成功，实际时间: {actual_time}s")
            else:
                ros_env.logerr(f"{cmd_name}指令执行失败（左: {ok_left}, 右: {ok_right}）")
        ...
        if success:
            ...
            ros_env.logerr(f"{cmd_name}指令执行失败")
        return success, actual_time
    ...
    def set_ruckig_params(self, 
        ...
        """
        try:
            ros_env.wait_for_service('/mobile_manipulator_set_ruckig_planner_params', timeout=5.0)
            client = ros_env.ServiceProxy('/mobile_manipulator_set_ruckig_planner_params', setRuckigPlannerParams)
            ...
            resp = client(req)
            if resp.result:
                ros_env.loginfo(f"✅ 规划器参数设置成功 (planner_index={planner_index}): {resp.message}")
                return True
            else:
                ros_env.logerr(f"❌ 规划器参数设置失败 (planner_index={planner_index}): {resp.message}")
                return False
        except ros_env.ROSException as e:
            ros_env.logerr(f"❌ 服务等待超时: {e}")
            return False
    ...
    def set_focus_ee(self, focus_ee: bool) -> None:
        ...
        """
        pub = ros_env.Publisher("/mobile_manipulator_focus_ee", Bool, queue_size=10, latch=True)
        pub.publish(Bool(data=bool(focus_ee)))
        ros_env.loginfo("笛卡尔跟踪焦点: %s", "末端(EE)" if focus_ee else "躯干(Torso)")
