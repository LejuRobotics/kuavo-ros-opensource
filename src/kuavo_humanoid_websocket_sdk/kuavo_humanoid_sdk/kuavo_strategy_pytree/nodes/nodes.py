            ...
            if self.exit_on_timeout:
                if callable(self.on_timeout_callable):
                    try:
                        ...
                        ros_env.logwarn(f"on_timeout_callable 执行异常: {e}")
                ros_env.logerr(self.timeout_message)
                sys.exit(1)
            return Status.FAILURE
        return Status.RUNNING
...
class NodeTagToBase(py_trees.behaviour.Behaviour):
    ...
    def _convert_tag_keypoints_to_base(self) -> bool:
        ...
        if tag is None or not hasattr(tag, "pose"):
            ros_env.logerr(f"NodeTagToBase: 黑板无 latest_tag_{self.tag_id}")
            return False
        ...
        if left_poses_base:
            ros_env.loginfo(f"[TAG->BASE] 左臂第1点 -> BASE: pos={left_poses_base[0].pos}")
        if right_poses_base:
            ros_env.loginfo(f"[TAG->BASE] 右臂第1点 -> BASE: pos={right_poses_base[0].pos}")
    ...
    def initialise(self):
        ...
        if (self.cmd_type in self._ARM_CMD_TYPES_WITH_FOCUS):
            if not ros_env.is_initialized():
                ros_env.init_node("node_wheel_move_timed_cmd", anonymous=True, disable_signals=True)
            self.focus_ee = True
...
class NodeWaitChassisReach(Behaviour):
    ...
    def update(self):
        ...
        if walk_goal is None:
            ros_env.logwarn_throttle(1.0, "NodeWaitChassisReach: 黑板无 walk_goal")
            return Status.RUNNING
        cur = self.robot_sdk.tools.get_link_pose(link_name="base_link", reference_frame=Frame.ODOM)
        if cur is None:
            ros_env.logwarn_throttle(1.0, "NodeWaitChassisReach: 获取底盘位姿失败")
            return Status.RUNNING
        ...
        if dx <= self.pos_tol and dy <= self.pos_tol and dyaw <= self.yaw_tol_rad:
            ros_env.loginfo(f"✅ 底盘到位: dx={dx:.3f}, dy={dy:.3f}, dyaw={np.rad2deg(dyaw):.2f}°")
            return Status.SUCCESS
        ros_env.logwarn_throttle(1.0, f"⚠️ 底盘未到位: erros_x={dx:.3f}, erros_y={dy:.3f}, erros_yaw={np.rad2deg(dyaw):.2f}° (容差 pos={self.pos_tol}, yaw={np.rad2deg(self.yaw_tol_rad):.2f}°)")
        return Status.RUNNING
    ...
    def update(self):
        if not self._interp_done:
            ...
            if t is not None and s is not None and float(t) > 0 and float(s) > 0:
                try:
                    ros_env.wait_for_service("/set_contact_force_params", timeout=2.0)
                    req = setContactForceInterpParamsRequest()
                    req.transition_time = float(t)
                    req.interpolation_speed = float(s)
                    ros_env.ServiceProxy("/set_contact_force_params", setContactForceInterpParams)(req)
                except (ros_env.ROSException, ros_env.ServiceException):
                    pass
        # 左右手各施加箱子重量的一半
...
class NodeHeadSearchTag(py_trees.composites.Sequence):
    ...
    def _log_from_blackboard(self, tag_id: int) -> bool:
        ...
        if tag is None or not hasattr(tag, "pose"):
            ros_env.logwarn(f"⚠️ 无法在黑板上读取 latest_tag_{tag_id}")
            sys.exit(1)
    ...
    def _validate_euler(
        ...
        if tag is None or not hasattr(tag, "pose") or not hasattr(tag.pose, "get_euler"):
            ros_env.logerr(f"❌ 审查 tag {tag_id}：黑板上无有效 latest_tag_{tag_id} 或 pose 无 get_euler")
            sys.exit(1)
        ...
        ros_env.loginfo(f"✅ Tag {tag_id} 欧拉角审查通过：{axes_str} 误差 {total_err:.2f}° < {threshold_deg}°")
        return True
    ...
    def _set_fake_to_blackboard(self) -> bool:
        ...
        ros_env.loginfo(f"✅ 虚假 tag 已设置: {[(c[0], c[1]) for c in tag_configs]}")
        return True
