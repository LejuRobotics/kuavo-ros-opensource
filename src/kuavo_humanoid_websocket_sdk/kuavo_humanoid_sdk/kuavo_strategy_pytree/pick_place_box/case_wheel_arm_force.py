...
def build_tree(timed_cmd_api: TimedCmdAPI) -> py_trees.behaviour.Behaviour:
    pub_enable_force_empty_detact = ros_env.Publisher("/enable_force_empty_detact", Bool, queue_size=10)
...
def main():
    ...
    ros_env.loginfo(f"期望力: {BOX_WEIGHT_KG} kg, 插值速度: {INTERPOLATION_SPEED}")
    ros_env.loginfo(f"手臂运动时间: {ARM_MOVE_TIME} s")
    ...
    while True:
        ...
        if status == py_trees.common.Status.SUCCESS:
            ros_env.loginfo("期望力测试完成!")
            break
        if status == py_trees.common.Status.FAILURE:
            ros_env.logerr("期望力测试失败.")
            break
        time.sleep(0.002)
