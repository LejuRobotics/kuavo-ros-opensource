...
def get_torso_initial_pose(need_pose: bool = True) -> Tuple[bool, Optional[dict]]:
    ...
    """
    try:
        ros_env.wait_for_service('/mobile_manipulator_get_torso_initial_pose', timeout=5.0)
        client = ros_env.ServiceProxy('/mobile_manipulator_get_torso_initial_pose', getLbTorsoInitialPose)
        ...
        if resp.result:
            ...
            ros_env.loginfo(f"✅ 获取躯干初始位姿成功")
            ros_env.loginfo(f"   位置: [{pose_data['position'][0]:.3f}, {pose_data['position'][1]:.3f}, {pose_data['position'][2]:.3f}]")
            ros_env.loginfo(f"   欧拉角(ZYX): [{pose_data['euler'][0]:.3f}, {pose_data['euler'][1]:.3f}, {pose_data['euler'][2]:.3f}]")
            return True, pose_data
        else:
            ros_env.logwarn(f"⚠️  获取躯干初始位姿失败: {resp.message}")
            return False, None
