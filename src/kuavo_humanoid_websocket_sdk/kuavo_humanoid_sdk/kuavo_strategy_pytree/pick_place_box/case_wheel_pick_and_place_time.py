...
def set_chassis_world_keypoints(
    ...
    ros_env.loginfo(f"📌 底盘关键点: [x={x:.3f}, y={y:.3f}, yaw={yaw:.3f}], time={time}s")
    return True
...
def run_tree(root: py_trees.behaviour.Behaviour):
    ...
    ros_env.loginfo("=" * 60)
    ros_env.loginfo("🚀 启动 PyTree: 搬箱子 (timed_cmd)")
    ros_env.loginfo("=" * 60)
...
def main():
    ...
    ros_env.loginfo(f"🦾 手臂控制: arm_ee_local 局部系")
    ros_env.loginfo(
        f"⏱️ 底盘时间: 抓取 {CHASSIS_PICK_TIME}s, 放置 {CHASSIS_PLACE_TIME}s; "
        f"手臂时间: 抓取 {ARM_PICK_TIME}s, 放置 {ARM_PLACE_TIME}s"
    )
    ros_env.loginfo(f"🏷️ 抓取 Tag: {PICK_TAG_ID}, 放置 Tag: {PLACE_TAG_ID}")
    if config.fake_tags.enable:
        ros_env.loginfo(f"📌 虚假 tag: pick={config.fake_tags.pick_pos}, place={config.fake_tags.place_pos}")
    ros_env.loginfo("")
