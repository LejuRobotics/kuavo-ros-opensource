...
def _reach_time_callback(msg: Float32):
    ...
    ros_env.loginfo(f"⏱️ /lb_cmd_pose_reach_time = {_LATEST_REACH_TIME:.3f} s")
...
def ensure_reach_time_subscriber():
    ...
    time_sub = ros_env.Subscriber("/lb_cmd_pose_reach_time", Float32, _reach_time_callback)
    _REACH_TIME_SUB_CREATED = True
    ros_env.loginfo("✅ 已订阅 /lb_cmd_pose_reach_time，用于动态设置手臂启动延时")
...
def log_tag_from_bb(tag_id: int):
    ...
    ros_env.loginfo(f"📍 tag {tag_id} 位置: {pos}")
    if quat is not None:
        ros_env.loginfo(f"   姿态(quat): {quat}")
    return True
...
def run_tree(root: py_trees.behaviour.Behaviour):
    ...
    ros_env.loginfo("=" * 60)
    ros_env.loginfo("🚀 启动 PyTree: 搬箱子 & 放箱子")
    ros_env.loginfo("=" * 60)

    while True:
        tree.tick()
        status = root.status
        ros_env.loginfo(py_trees.display.unicode_tree(root, show_status=True))

        if status == py_trees.common.Status.SUCCESS:
            ros_env.loginfo("=" * 60)
            ros_env.loginfo("✅ 任务完成：搬起并放下箱子!")
            ros_env.loginfo("=" * 60)
            break
...
def main():
    ...
    if not ros_env.is_initialized():
        ros_env.init_node("pick_place_case_node", anonymous=True, disable_signals=True)
    ensure_reach_time_subscriber()

    ros_env.loginfo(f"🎮 移动控制模式: {args.walk_mode}")
    ros_env.loginfo(f"🦾 手臂控制类型: {ARM_CONTROL_TYPE}")
    ros_env.loginfo(f"🏷️ 抓取 Tag ID: {PICK_TAG_ID}, 放置 Tag ID: {PLACE_TAG_ID}")
    ros_env.loginfo("")
