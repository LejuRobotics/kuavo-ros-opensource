...
EC_T_DWORD EcDemoApp(T_EC_DEMO_APP_CONTEXT *pAppContext)
{
...

  {
    ...
    while (bRun)
    {
      if (motorEnable(g_motor_info[i].logical_id+1))
      {
...
static EC_T_DWORD myAppPrepare(T_EC_DEMO_APP_CONTEXT* pAppContext)
{
    ...
    // snum_slave = dwNumConfiguredSlaves;//总的从站数目
    //初始化从站计数器
    num_elmo_slave = 0;
    num_yd_slave = 0; 
  ...
  //根据不同的厂商的id统计从站数目 （Vendor 厂商ID）
    ...
    {
      g_motor_info[motor_count].physical_id = motor_count;
      g_motor_info[motor_count].slave_id = dwSlaveIdx+1;
      g_motor_info[motor_count].logical_id = slaveTojoint(dwSlaveIdx);
      g_motor_info[motor_count].pdo_id = num_yd_slave;
      g_motor_info[motor_count].driver_type = YD;
      num_yd_slave++;
      motor_count++;
    ...
    {      
      ...
      g_motor_info[motor_count].slave_id = dwSlaveIdx+1;
      g_motor_info[motor_count].logical_id = slaveTojoint(dwSlaveIdx);
      g_motor_info[motor_count].pdo_id = num_elmo_slave;
      g_motor_info[motor_count].driver_type = ELMO;
      num_elmo_slave++;
      motor_count++;
  ...
  // printf("[ECmaster]: 开始按logical_id排序g_motor_info数组，共%d个电机:\n", num_motor_slave);

  // 使用插入排序按logical_id排序
  for (uint32_t i = 1; i < num_motor_slave; i++) {
    MotorInfo_t key = g_motor_info[i];
    uint32_t j = i;
    
    while (j > 0 && g_motor_info[j - 1].logical_id > key.logical_id) {
      g_motor_info[j] = g_motor_info[j - 1];
      j--;
    }
    g_motor_info[j] = key;
  }
  ...
  //   printf("[ECmaster]: g_motor_info[%d] -> slave_id=%d, logical_id=%d, pdo_id=%d, driver_type=%d, physical_id=%d\n", 
  //          i, g_motor_info[i].slave_id, g_motor_info[i].logical_id, g_motor_info[i].pdo_id, g_motor_info[i].driver_type, g_motor_info[i].physical_id);
  // }
...
static EC_T_DWORD myAppSetup(T_EC_DEMO_APP_CONTEXT* pAppContext)
{
    ...
    {
      setPdoRwEn(false);
      real_config_parameter = getRobotAllDriverConfigParameter(num_motor_slave);
    }
    //写入驱动器配置
    else if(getSdoStatus() == SDO_STATUS_WRITE_ALL)
    {
      setPdoRwEn(false);
      Sdo_RW_Res = robotSetAllConfig(num_motor_slave, g_conJsonPath.c_str()); 
    }
    //单个配置读取
    else if(getSdoStatus() == SDO_STATUS_READ_SINGLE)
    {
      setPdoRwEn(false);
      Sdo_RW_Res = readSingleSdo((SdoRead.SlaveId-1),SdoRead.ObIndex,SdoRead.SubIndex,&SdoRead.read_data);
    }
    //单个配置写入
    else if(getSdoStatus() == SDO_STATUS_WRITE_SINGLE)
    {
      setPdoRwEn(false);
      Sdo_RW_Res = writeSingleSdo((SdoWrite.SlaveId-1),SdoWrite.ObIndex,SdoWrite.SubIndex,&SdoWrite.write_data,SdoWrite.save);
    }
    //电机自学习
    else if(getSdoStatus() == SDO_STATUS_SELF_LEARN)
    {
      setPdoRwEn(false);
      Sdo_RW_Res = motorSelfLearning(SdoWrite.SlaveId-1,SdoWrite.write_data,20);
    }
    else if(getSdoStatus() == SDO_STATUS_READ_ERROR_CODE)
    {
      setPdoRwEn(false);
      std::vector<int32_t> error_code_list;
      Sdo_RW_Res = readAllErrorCode(num_motor_slave,error_code_list);
      if(Sdo_RW_Res == true)
      {
        checkErrorCode(error_code_list);
      }
    }
    else
    {
