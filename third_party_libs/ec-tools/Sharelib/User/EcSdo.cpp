...
namespace FieldRegistry {
    const std::vector<JsonFieldDescriptor> ALL_FIELD_DESCRIPTORS = {
        ...
        FIELD_DESC_DOUBLE(driverRatingCount, "A", 100, "%.1f", DRIVER_RATING_COUNT, 0),
        ...
        FIELD_DESC_DOUBLE(PrE_02, "", 100, "%.1f", DefPrE_02, 0),
        FIELD_DESC_DOUBLE(PrE_04, "", 100, "%.1f", DefPrE_04, 0),
        FIELD_DESC_DOUBLE(PrE_05, "", 1, "%u", DefPrE_05, 0),
        FIELD_DESC_DOUBLE(PrE_06, "", 1, "%u", DefPrE_06,0),
        FIELD_DESC_DOUBLE(PrE_07, "", 1, "%u", DefPrE_07, 0),
        FIELD_DESC_DOUBLE(PrE_08, "", 1, "%u", DefPrE_08, 0),
        FIELD_DESC_DOUBLE(PrE_12, "", 10, "%.1f", DefPrE_12, 0),
        FIELD_DESC_DOUBLE(PrE_14, "", 100, "%.1f", DefPrE_14, 0),
        FIELD_DESC_DOUBLE(PrE_21, "", 1, "%u", DefPrE_21, 0)
    };
...
static const std::map<int32_t, std::string> errorMsgMap = {
    ...
    {0x036, "电机自学习完成(模式3)  处理方式：重启驱动器"},
    {0x01F, "电机自学习完成(模式1)  处理方式：重启驱动器"},
    {0x082, "驱动器温度大于等于100℃  处理方式：检查驱动器散热，通过0x300F查看驱动器温度是否正常"}
};
...
std::vector<EC_DriversConfigParameter_t> getRobotAllDriverConfigParameter(const uint8_t all_num_slave)
{
  ...
  for (uint8_t i = 0; i < all_num_slave; i++)
  {
    ...
    for (const auto& field_name : read_fields)
    {
      ...
      {
        if (!readSingleSdo(g_motor_info[i].slave_id-1, field_desc->obIndex, field_desc->subIndex, &temp_value))
        {
          printf("Joint %d, Slave %d 读取 %s 失败\n", g_motor_info[i].logical_id+1, g_motor_info[i].slave_id, field_name.c_str());
          return {};
        }
  ...
  for (uint32_t i = 0; i < all_num_slave; i++)
  {
    ...
    for (const auto& field : FieldRegistry::getFieldDescriptors(FieldNameList::WRITE_FIELD_NAMES))
    {
      if (field && field->obIndex != 0)
      {
        ...
        if (EC_E_NOERROR != emCoeSdoDownload(0, g_motor_info[i].slave_id-1, field->obIndex, field->subIndex, buf_32_t, 2, 100, 0)){
          printf("Failed to emCoeSdoDownload Joint: %d, Slave: %d, field: %s\n", g_motor_info[i].logical_id+1, g_motor_info[i].slave_id, field->name.c_str());
          return false;
        }
      }
    }
    toLittleEndian32(to_save, buf_32_t);
    if (EC_E_NOERROR != emCoeSdoDownload(0, g_motor_info[i].slave_id-1, DRIVER_SAVE_SETTING_PARAMETER, 0, buf_32_t, 2, 100, 0)){
      printf("Joint: %d, Slave: %d, Failed to save emCoeSdoDownload!\n", g_motor_info[i].logical_id+1, g_motor_info[i].slave_id);
      return false;
    }
    printf("Joint: %d, Slave: %d, Save emCoeSdoDownload successfully!\n", g_motor_info[i].logical_id+1, g_motor_info[i].slave_id);
  }
  return true;
}
// ==============================================
...
bool motorSelfLearning(uint8_t slave_id, int32_t learn_mode, uint8_t timeout_seconds)
{
  ...
  {
    printf("Joint %d, Slave %d 读取错误码失败\n", (slaveTojoint(slave_id)+1), (slave_id+1));
    return false;
  }
  if(error_code == MOTOR_SELF_LEARN_COMPLETE_FLAG1 || error_code == MOTOR_SELF_LEARN_COMPLETE_FLAG3)
  {
    printf("Joint %d, Slave %d 已自学习过，请勿重复自学习\n", (slaveTojoint(slave_id)+1), (slave_id+1));
    return false;
  }

  if (!writeSingleSdo(slave_id,MOTOR_SELF_LEARN_PARAMETER,0,&write_data,false))
  {
    printf("Joint %d, Slave %d 写入自学习参数失败\n", (slaveTojoint(slave_id)+1), (slave_id+1));
    return false;
  }
  while(1)
  {
    ...
    else if(error_code != 0)
    {
      ...
      printf("Joint %d, Slave %d 存在错误码: 0x%04X\n", (slaveTojoint(slave_id)+1), (slave_id+1), error_code);
    ...
    {
      printf("Joint %d, Slave %d 自学习超时\n", (slaveTojoint(slave_id)+1), (slave_id+1));
      return false;
    }
    sleep(1);
    printf("Joint %d, Slave %d 正在自学习...\n", (slaveTojoint(slave_id)+1), (slave_id+1));
  }
  printf("Joint %d, Slave %d 自学习结束!\n", (slaveTojoint(slave_id)+1), (slave_id+1));
  // printf("error_code: %d\n", error_code);
  return true;
}
...
bool readAllErrorCode(uint8_t all_num_slave,std::vector<int32_t> &error_code_list)
{
  ...
  for (uint8_t i = 0; i < all_num_slave; i++)
  {
    if(!readSingleSdo(g_motor_info[i].slave_id-1,INDEX_ERROR_CODE,0,&error_code))
    {
      printf("Joint %d, Slave %d 读取错误码失败\n", (g_motor_info[i].logical_id+1), g_motor_info[i].slave_id);
      return false;
    }
...
uint8_t checkErrorCode(const std::vector<int32_t> &error_code_list)
{
  ...
  for (size_t i = 0; i < error_code_list.size(); ++i)
  {
    ...
    if (error_code == 0)
      continue;
    printf("\033[1;31m[错误]\033[0m ");
    printf("关节: \033[1;33m%-2u\033[0m, 从站: \033[1;33m%-2u\033[0m | 错误码: \033[1;31m0x%04X\033[0m\n", g_motor_info[i].logical_id+1, g_motor_info[i].slave_id, error_code);
    auto it = errorMsgMap.find(error_code);
    if (it != errorMsgMap.end()) {
      printf("  \033[1;36m说明: %s\033[0m\n", it->second.c_str());
    }
